...
def set_device(config, rank):
    if config.DDP:
        ...
        config.gpu_num = torch.cuda.device_count() if torch.cuda.is_available() else 1
        if torch.cuda.is_available() and config.gpu_num > 1:  # Only multiply batch size for multi-GPU
